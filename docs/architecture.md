...
| `roi_config.py` | ROI 参数默认值、JSON 读取、范围校验和稳定快照 | 推断实际费率或物流报价 |
| `http_retry.py` | 对传输错误、429 和 5xx 进行有限重试 | 重试认证失败、普通 4xx 或业务协议错误 |
...
- 自定义 ROI 参数先经过严格校验，并随结果记录来源和快照。
- 采集检查点只保存候选对象，不保存 Cookie 或请求头。
