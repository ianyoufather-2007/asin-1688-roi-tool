...
- [ ] 写配置默认值、覆盖和无效输入的失败测试。
- [ ] 运行目标测试并确认因模块或接口缺失而失败。
- [ ] 实现配置对象和 JSON 加载。
- [ ] 写 ROI 自定义参数与追溯输出的失败测试。
- [ ] 接入 ROI、CLI、工作流和 Excel。
- [ ] 运行相关测试直至通过。
...
- [ ] 写 CLI 参数和错误提示的失败测试。
- [ ] 实现日志、提示和检查点编排。
- [ ] 限制 CI push 为 `main`，增加并发取消。
- [ ] 增加 Windows 安装脚本、参数文档和发布清单。
- [ ] 校验 Markdown 链接、YAML 结构和脚本语法。
